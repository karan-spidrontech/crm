...
$bs_symbol = $this->_instance->currencies_model->get_base_currency()->symbol;
...
foreach ($rResult as $aRow) {
    ...
    for ($i=0 ; $i<count($aColumns) ; $i++) {
        ...
        } elseif ($aColumns[$i] == 'dateend' || $aColumns[$i] == 'datestart') {
            $_data = _d($_data);
        ...
        } elseif ($aColumns[$i] == 'contract_type') {
            $_data = $aRow['name'];
        } elseif ($aColumns[$i] == 'contract_value') {
            $_data = format_money($_data, $bs_symbol);
        } else {
