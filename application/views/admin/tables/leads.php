...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if ($aColumns[$i] == '1') {
        ...
        } elseif ($aColumns[$i] == 'tblleads.name' || $aColumns[$i] == 'tblleads.id') {
            $_data = '<a href="'.admin_url('leads/index/'.$aRow['id']).'" onclick="init_lead('.$aRow['id'].');return false;">'. $_data . '</a>';
        } elseif ($aColumns[$i] == 'lastcontact' || $aColumns[$i] == 'dateadded') {
            ...
            if ($aRow['assigned'] != 0) {
                $full_name = $_data;
                $_data = '<a data-toggle="tooltip" data-title="'.$full_name.'" href="'.admin_url('profile/'.$aRow['assigned']).'">'.staff_profile_image($aRow['assigned'], array(
                    'staff-profile-image-small'
                    )) . '</a>';
                // For exporting
                $_data .= '<span class="hide">'.$full_name.'</span>';
            } else {
                $_data = '';
            }
        } elseif ($i == 6) {
            $_data = render_tags($_data);
        } elseif ($aColumns[$i] == 'tblleadsstatus.name') {
            if ($aRow['tblleadsstatus.name'] == null) {
                ...
                $_data = '<span class="inline-block label'.(!$this->_instance->input->post('status') ? ' pointer lead-status' : '').' label-' . $_label_class . '" style="color:' . $aRow['color'] . ';border:1px solid ' . $aRow['color'] . '">' . $_data . '</span>';
            }
        } elseif ($aColumns[$i] == 'tblleads.phonenumber') {
            $_data = '<a href="tel:' . $_data . '">' . $_data . '</a>';
        } elseif ($aColumns[$i] == 'tblleads.email') {
