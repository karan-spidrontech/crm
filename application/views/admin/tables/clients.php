...
$aColumns = array(
    ...
    'tblclients.active',
    '(SELECT GROUP_CONCAT(name ORDER BY name ASC) FROM tblcustomersgroups LEFT JOIN tblcustomergroups_in ON tblcustomergroups_in.groupid = tblcustomersgroups.id WHERE customer_id = tblclients.userid)'
);
...
foreach ($this->_instance->projects_model->get_project_statuses() as $status) {
    if ($this->_instance->input->post('projects_' . $status['id'])) {
        array_push($_projects, $status['id']);
    }
}
...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        } elseif ($aColumns[$i] == 'tblclients.phonenumber') {
        ...
        } elseif ($aColumns[$i] == 'tblclients.active') {
            $checked = '';
