       ...
       <div class="row">
        <div class="col-md-8 border-right project_file_area">
          ...
          if(empty($file->external)) {
            ...
            } elseif(!empty($file->external) && $file->external == 'dropbox') {
              // Direct download link on filename when file is linked to dropbox and no previs is available.
              echo '<a href="'.str_replace('dl=0', 'dl=1', $file->external_link).'">'.$file->file_name.'</a>';
          }
          $path = get_upload_path_by_type('project') . $file->project_id . '/'. $file->file_name;
          if(is_image($path)){ ?>
          <img src="<?php echo site_url('download/preview_image?path='.protected_file_url_by_path($path).'&type='.$file->filetype); ?>" class="img img-responsive">
          ...
          <?php } else if(strpos($file->filetype,'pdf') !== false){ ?>
