...
class Utilities_model extends CRM_Model
{
    ...
    public function get_calendar_data($client_id = '', $contact_id = '', $filters = false)
    {
        $is_admin                     = is_admin();
        ...
        if (get_option('show_tasks_on_calendar') == 1 && !$ff || $ff && array_key_exists('tasks', $filters)) {
            ...
            $this->db->select('duedate,startdate,name as title,id,rel_id,rel_type,status');
